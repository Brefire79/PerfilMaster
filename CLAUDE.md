...
- **Bypass em trigger SECURITY DEFINER (DELTA 27, 19/09/2026)**: dentro de função `SECURITY DEFINER`, `current_user` é o **dono** (postgres) — `IF current_user IN ('postgres', …)` é sempre verdadeiro. `protect_user_privileges` e `protect_admin_deletion` tinham esse bypass (aluno com UPDATE na própria linha conseguia `role='admin'`); corrigidos para **`session_user`** (`authenticator` no PostgREST). O contrato de segurança rejeita `current_user IN (` em migrations novas. **Use `session_user` + `auth.role()`** em qualquer trava nova.
...
- Tabelas: `app_users`, `app_groups`, `app_modules`, `app_assessments`, `app_profiles`, `app_invites`, `app_sessoes`, `app_avaliados`, `app_sessao_respostas`, `app_group_reports`, `app_identity_links`, `app_superadmins`, `audit_log`, `app_central_ai` (Central, DELTA 14/16), `app_client_errors` + `app_rate_limit` (DELTA 20), `app_profiles_historico` (DELTA 25), `app_ciclos` (DELTA 26).
...
- **Mestre v2 (19/09/2026)** — o roteador por regex virou **intenção + entidades** (`src/lib/mestreIntencao.js`, puro, travado por `scripts/verify-mestre-contract.mjs`): casa nomes reais de **grupos e pessoas** do escopo (`catalogoDoEscopo`), período e sabotador; **slot-filling** quando falta a pessoa/turma ("Para quem?") ou o nome é ambíguo (lista os candidatos). Consultas novas em `src/lib/mestreConsultas.js`: `pendencias` (quem não concluiu + testes vencidos/vencendo), `abordagem_turma` (motor agregado), `abordagem_pessoa` e `evolucao_pessoa` (**pelo nome** — decisão de produto: o facilitador já vê esses dados nas telas; registra `admin_viewed_history` via `logAudit`), `ciclos` (testes dirigidos por status/módulo). **Miss-log no servidor**: `registrarMiss` também manda `mestre_miss` ao `audit_log` via `logAudit` (allowlist atualizada — **requer redeploy de `logAudit`**); a aba Diagnóstico ganhou o card *Perguntas que o Mestre não entendeu* (`components/central/MestreMisses.jsx`; superadmin vê todos os facilitadores). Base de conhecimento ampliada (testes dirigidos, motor, linha do tempo, comparativo, janela, turma empresarial). Continua **sem IA externa**.
- **Mestre** (ex-Módulo 4): desde jul/2026 é um **chat flutuante 100% local** — a sub-aba Central › "Mestre (IA)" foi **removida** (rota `central/assistente` redireciona ao Painel; `AssistenteIA.jsx` preservado sem rota). Gatilho "Perguntar ao Mestre" no header do **Dashboard**; painel (`src/components/mestre/MestreChat.jsx`) montado no `AdminLayout`. Motor `src/lib/mestreLocal.js`: roteia por palavras-chave para as consultas fixas (`inteligencia_grupos`/`visao_geral`/`saude_status`/`contagem` — esta última replica o cálculo do card "Total de Alunos" do Painel, com dedup de convertidos DELTA 19) via os mesmos RPCs escopados das outras abas + base de conhecimento do app (modo conversa, incl. data/hora local) — **nenhuma chamada a IA externa no chat**. A API DeepSeek fica reservada à análise nos relatórios (`insightPerfil`) e ao pipeline de avaliação. **Conversa persistida** em `mestreStore` (localStorage `profileai.mestre.chat`), limpa só no logout (`authStore.clearUser`). **Miss-log** (`profileai.mestre.misslog`, localStorage): perguntas sem resposta + erros, insumo para evoluir o vocabulário; não é limpo no logout. Edge `assistenteCentral` e `AssistenteIA.jsx` **removidos em 19/09/2026**. PDF local (`src/lib/centralPdf.js`, import dinâmico).
...
- Requer no painel do Supabase: *Providers → Google* (Client ID/Secret do Google Cloud, grátis) e *Redirect URLs* com `{origin}/auth/callback`. O botão só aparece com `VITE_ENABLE_GOOGLE_AUTH=true` **no `.env.local` da máquina que roda `npm run deploy`** (o build é local; env do painel Netlify não entra no bundle). Escondido por padrão — `GoogleButton.jsx`.
...
### Devolutiva por e-mail (18/09/2026)
Ao concluir uma avaliação, **duas** mensagens saem via Resend (`_shared/email.ts` → `sendEmail`, templates e roteamento em `_shared/devolutiva.ts` → `notificarConclusao`):
- **Avaliado**: perfil dominante/secundário, barras DISC, PQ, resumo + pontos fortes da IA (quando houver) e botão "Ver perfil completo" (`/student/profile` no fluxo de conta; `/resultado/:token` no público). Só se tiver e-mail (`app_users.email` / `app_avaliados.email`).
- **Facilitador**: "Fulano concluiu — perfil X/Y" com o mesmo resumo e link para o membro (`/admin/relatorio/aluno/:uid` ou `/admin/relatorio/:token`). Respeita `app_users.notifications.assessmentComplete` (ausente = ligado); não envia se for o mesmo e-mail do avaliado.
- Disparo: `buildProfile` (fluxo de conta — roda em background após o wizard gravar `app_profiles`, então o e-mail chega ~20-30 s depois com o texto da IA; se a IA falhar, envia sem resumo) e `atualizarStatus` (fluxo público, ao `concluido`). Facilitador do aluno = `app_groups.adminuid`, senão `app_users.adminuid`.
- **Best-effort por desenho**: sem `RESEND_API_KEY` ou com o Resend fora, o log avisa e a resposta segue `200` — perfil gravado nunca depende de e-mail. A resposta traz `devolutiva: { avaliado, facilitador }` (booleans) para diagnóstico.
...
- **Comparativo da turma (DELTA 24, 19/09/2026)** — aba *Comparativo* em Grupos › grupo (`components/group/ComparativoTurma.jsx`, `?tab=compare`): uma linha por pessoa que concluiu, juntando **conta** (`app_profiles` dos membros) e **celular** (`app_avaliados` concluídos das sessões com `groupid` do grupo, ignorando convertidos). Colunas Pessoa / Perfil (dom + sec) / D / I / S / C / PQ / Concluído, ordenáveis; chips de distribuição por dominante que filtram; média no rodapé; **Exportar CSV** (BOM + `;`, abre em colunas no Excel pt-BR, com linha de média). Perfil de conta cujo `app_profiles.groupid` ≠ grupo aparece com badge *perfil anterior* e pode ser excluído por checkbox. Só leitura no cliente — nada novo no banco.

### Janela de horário da avaliação (DELTA 23, 19/09/2026)
"Todos respondem juntos": `app_groups.janela_inicio` / `janela_fim` (aplicado no banco em 19/09/2026). A janela governa o **início**; quem começou dentro pode terminar até **fim + 2 h** (`TOLERANCIA_MS` em `src/lib/janela.js` = `janela_permite_envio()` no banco = checagem em `atualizarStatus`).
- **Admin**: card *Janela da avaliação* na aba Convite (`components/group/JanelaAvaliacao.jsx`, `datetime-local`, horário do aparelho) e campos opcionais no modal *Turma empresarial*. Frase de estado ("Abre em 2d 03h", "Aberta — fecha em…", "Encerrada").
- **Participante**: `components/assessment/JanelaGate.jsx` (`useJanela` + cartão com contagem regressiva de 1 s / "encerrada"), usado no `AssessmentWizard` (lê o grupo do aluno) e em `AvaliacaoPublica` (janela vem de `buscarPorToken` via `app_sessoes.groupid`). `/join` mostra "Avaliação: abre em…" (`validateInviteToken` devolve `janela`).
...
### Testes Dirigidos + motor de abordagem (Fase 2, 19/09/2026)
- **Catálogo** `src/constants/testesDirigidos.js`: 7 testes (`TD-ASSERTIVIDADE`, `TD-DELEGACAO`, `TD-EQUILIBRIO`, `TD-DECISAO`, `TD-FOCO`, `TD-FEEDBACK`, `TD-PQ-BASE`), cada um com 12 itens likert5 em 3 subescalas × 4, **com itens invertidos** (`invertido: true` → `6 − valor`) — concordar com tudo dá 50, não 100. `versao` por teste; mudou item, sobe a versão (resultados só se comparam na mesma versão).
- **Scoring** `src/lib/testeDirigidoScoring.js` (`pontuarTesteDirigido(codigo, respostas)` → subescalas 0–100 + `geral`). É a referência; quando for pontuado no servidor, espelhar e travar no contrato.
- **Motor** `src/lib/abordagem.js` — `sugerirAbordagem(perfil, { jaAplicados })` → `{ regraId, versao, modulo, foco, prazoDias, justificativa[], alternativas[], alertas[] }`. Regras: R1 aquiescência (≥3 DISC ≥70 → alerta), R2 PQ < 60 → `TD-PQ-BASE`, R3 sabotador ≥ 65 (mais intenso decide; mapa `SABOTADOR_PARA_TESTE`), R4 só-DISC, R5 padrão, R6 rotação (já aplicado → próximo). `sugerirAbordagemTurma(perfis, minN)` agrega com k-anonimato. Determinístico e auditável — a IA só redige, nunca decide. Aceita os 3 formatos de perfil do app (`normalizarPerfil`).
- **Contrato** `scripts/verify-testes-dirigidos-contract.mjs` (no `npm test`).
- **UI**: `components/profile/ProximaAbordagem.jsx` = § 3.3 do Relatório Oficial (só leitura). `getAvaliadoLikeFromUid` agora expõe `perfil.saboteurScores`/`saboteurTop3` para contas (antes a § 3.2 e o motor viam a conta como DISC-only).
- **Aplicação pelos 3 canais (DELTA 26, banco aplicado 19/09/2026)** — tabela **`app_ciclos`** (um Teste Dirigido aplicado a uma pessoa: `pessoa_tipo` conta|avulso, `token` UUID = credencial do link, `modulo_codigo/versao`, `regra_id/motor_versao/justificativa` do motor, `status` aplicado→concluido|descartado, `respostas`, `resultado`, `prazo_em`). RLS: facilitador tudo nos seus (`adminuid`); aluno SELECT nos seus e UPDATE **só** de `respostas/resultado/status→concluido` (trigger `app_ciclos_protege_colunas`); anon nada. **Auditoria por trigger** `app_ciclos_audit` → `audit_log` (`cycle_applied`/`cycle_completed`/`cycle_discarded`, metadata sem PII) — cobre os 3 canais sem depender do cliente.
  - **Conta**: `/student/teste/:id` (`pages/student/TesteDirigido.jsx`, scoring no cliente, rascunho em `localStorage` `profileai.td.respostas.<id>`); card "Próximo passo" no Início do aluno.
  - **Link**: `/teste/:token` (`pages/public/TesteDirigidoPublico.jsx`) → Edge `cicloPorToken`/`cicloResponder` (scoring no servidor).
  - **Turma**: Grupos › Comparativo › card *Próximo foco da turma* (`components/group/AbordagemTurma.jsx`) — agrega o motor e aplica a todos os concluídos (pula quem já tem o mesmo teste aguardando); lista links WhatsApp dos avulsos.
  - **Individual**: Relatório Oficial › § 3.3 + `components/profile/AplicarTesteDirigido.jsx` (select do teste, sugerido marcado, lista de aplicações, WhatsApp/copiar link, descartar). Helpers em `firestore.js`: `criarCiclo`, `getCiclosDaPessoa`, `getCiclosByAdmin`, `getCiclosByUids`, `getCiclo`, `responderCiclo`, `descartarCiclo` (colunas snake_case explícitas, fora do `CAMEL_TO_DB`).
  - **Espelho Deno do catálogo é GERADO**: `node scripts/gen-testes-dirigidos-edge.mjs` regrava `supabase/functions/_shared/testesDirigidos.ts`; o contrato falha se estiver desatualizado. Mudou item → rode o gerador.
...
- **Mestre › "Aprofundar com IA"** (opcional, por mensagem com dados) — Edge `mestreAprofundar` (JWT admin, rate limit 30/h em `app_central_ai`): recebe pergunta + `dados` já calculados localmente, **anonimiza no servidor** (`PII_KEYS` + listas nominais viram contagens + nome do grupo vira "a turma"), DeepSeek redige interpretação e 2–4 ações; a narrativa local (com nomes) **não** é enviada. Substituiu `assistenteCentral` (código removido em 19/09/2026; a tabela `app_central_ai` continua como rate limit/registro). O chat continua local por padrão.
...
- [x] **DISC-V2 — itens invertidos (19/09/2026, DELTA 29 aplicado no banco)**: `q_d/i/s/c_02` e `_07` reescritos como invertidos (`invertido: true`, `6 − valor`), mesmos ids/pesos; `DISC_VERSAO = 2` em `sampleQuestions.js`, `discScoring.js`, `atualizarStatus` e `_shared/disc.ts`; `app_profiles.disc_versao` (default 1 para os 14 antigos; wizard grava 2) e `perfil.discVersao` no fluxo público; Linha do Tempo / Meu Perfil avisam "comparação aproximada" entre versões; § 2 do relatório marca "questionário v2"; chaves de rascunho versionadas (`…respostas.v2.…`). Contrato: extremos = melhor/pior resposta, concordar com tudo dá 75 (antes 100), 8 invertidos obrigatórios nos 2 espelhos. **Requer redeploy de `atualizarStatus` + `npm run deploy` juntos** (front e Edge têm de trocar de versão ao mesmo tempo).
...
- [x] **Devolutiva por e-mail (18/09/2026)** — ver seção "Devolutiva por e-mail" acima. Código pronto; **requer**: secrets `RESEND_API_KEY` + `EMAIL_FROM` (domínio verificado no Resend) + `APP_URL` no Supabase, e redeploy de `buildProfile` e `atualizarStatus`. Push e digest semanal continuam só como preferência salva.
...
- `en.json` e `es.json` apagados em 19/09/2026.

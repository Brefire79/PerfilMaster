...
### 6.10.1 Ciclo de desenvolvimento — histórico, motor de abordagem e Testes Dirigidos (19/09/2026)
- **Histórico (DELTA 25)**: `app_profiles` é o perfil atual; refazer a avaliação copia o anterior para `app_profiles_historico` (trigger, só quando o `assessmentid` muda) e incrementa `ciclo`. Linha do Tempo em Central › Pessoas & Histórico e "Sua evolução" em Meu Perfil › Histórico, com Δ de D/I/S/C e PQ. `disc_versao` por perfil (DELTA 29).
- **Motor de abordagem** (`src/lib/abordagem.js`, determinístico, versionado): R1 aquiescência (alerta), R2 PQ < 60 → Fundamentos, R3 sabotador ≥ 65 → teste correspondente, R4 só DISC, R5 padrão, R6 rotação. Sugestão com `regraId` + versão. Agregação por turma.
- **Testes Dirigidos** (`src/constants/testesDirigidos.js`): 7 × 12 itens likert5, 3 subescalas, metade invertida; espelho Deno gerado. Aplicação (DELTA 26, `app_ciclos`): **conta** (`/student/teste/:id`, card no Início), **link** (`/teste/:token`, Edge `cicloPorToken`/`cicloResponder` com scoring no servidor), **turma** (Grupos › Comparativo › *Próximo foco da turma*). Auditoria por trigger (`cycle_applied/completed/discarded`). Relatório Oficial § 3.3 + `AplicarTesteDirigido`.
- **Lembretes** (DELTA 28): Edge `enviarLembretesCiclos` — cron semanal (`CRON_TOKEN`) ou botão na turma; Resend; `lembrete_em` evita repetição.
- **Social Style** (`src/lib/socialStyle.js`): lente derivada do DISC, § 2.1 do relatório e card no Meu Perfil.
- **DISC‑V2**: 8 dos 28 itens invertidos (`q_?_02`, `q_?_07`), `DISC_VERSAO = 2` nos dois motores; "concordar com tudo" passa de 100 para 75.
...
Assistente do facilitador, **local por padrão** — nenhuma chamada a IA externa no chat, exceto o botão opcional *Aprofundar com IA* (v2). A API de IA (DeepSeek) fica reservada à análise nos relatórios (`insightPerfil`), ao pipeline de avaliação e a esse aprofundamento.

- **Mestre v2 (19/09/2026)**: interpretador de **intenção + entidades** (`src/lib/mestreIntencao.js`) casa nomes reais de grupos e pessoas do escopo, período e sabotador, com slot-filling ("Para quem?") e desambiguação. Consultas novas (`src/lib/mestreConsultas.js`): `pendencias`, `abordagem_turma`, `abordagem_pessoa`, `evolucao_pessoa` (pelo nome — decisão de produto; registra `admin_viewed_history`), `ciclos`. **Miss-log no servidor** (`audit_log` · `mestre_miss`) com card em Central › Diagnóstico. **Aprofundar com IA**: Edge `mestreAprofundar` anonimiza no servidor (PII removida, listas nominais viram contagens) antes de chamar a IA; rate limit 30/h em `app_central_ai`. `assistenteCentral` foi removida.
...
| 1.0.78 | 19 Set 2026 | **Plano de evolução entregue**: histórico por ciclo + Linha do Tempo (DELTA 25); motor de abordagem + 7 **Testes Dirigidos** pelos 3 canais com `app_ciclos` e auditoria por trigger (DELTA 26); **hotfix de segurança** no bypass `current_user` de triggers SECURITY DEFINER (DELTA 27); lembretes por e-mail com cron (DELTA 28); **Mestre v2** (intenção + entidades, pessoas pelo nome, miss-log, aprofundar com IA); **Social Style** derivado; **DISC‑V2** com 8 itens invertidos (DELTA 29); 5 contratos no `npm test`; `assistenteCentral` removida. Plano: `PLANO-EVOLUCAO-2026-09-19.md` |
